...
class AirSimCarEnv(AirSimEnv):
    ...
    def step(self, action):
        """Apply action and return Gymnasium-compatible tuple.
        ...
        # Map done to terminated; no explicit truncation mechanism here
    ...
    def reset(self, *, seed=None, options=None):
        """Reset environment. Compatible with Gymnasium's reset signature.

        Returns (obs, info)
        """
        # Gymnasium may pass an RNG seed; accept but we don't use it here
        if seed is not None:
            try:
                # set seed for numpy RNG if desired
                np.random.seed(seed)
            except Exception:
                pass
